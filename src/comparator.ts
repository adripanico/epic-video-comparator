import {
  ...
  Player,
  PlayerClassType,
} from '@epiclabs/epic-video-player';
...
export class Comparator {
  ...
  constructor(
    public config: IComparatorConfig,
  ...
  public play(): void {
    void this.leftPlayer.play();
    void this.rightPlayer.play();
    this.hideSpinner();
  }
  ...
  public toggleFullScreen(): void {
    ...
    if (this.isFullScreen) {
      ...
      // TODO investigate error screen.orientation.lock() is not available on this device.
      ...
      // TODO investigate error screen.orientation.lock() is not available on this device.
      // void screen.orientation.lock?.('landscape-primary');
    }
    this.resizePlayers();
  }
   ...
   */
  public setRenditionKbps(
    player: 'left' | 'right' | Player<PlayerClassType>,
    kbps: number,
  ): IRendition {
    return this.setRenditionByKbps(player, kbps);
  }

  public setRenditionByKbps(
    player: 'left' | 'right' | Player<PlayerClassType>,
    kbps: number,
  ): IRendition {
  ...
  public toggleStats(): void {
    const leftStatsContainer = this.container.getElementsByClassName(
      `${Comparator.LIB_PREFIX}left-stats`,
    )[0] as HTMLDivElement;
    leftStatsContainer.classList.toggle('hidden');
    const rightStatsContainer = this.container.getElementsByClassName(
      `${Comparator.LIB_PREFIX}right-stats`,
    )[0] as HTMLDivElement;
  ...
  private updateStatsBox(
    ...
    if (
      ...
      (this.config.stats as IStatsConfig).custom === true
    ) {
      return;
    }
    ...
    if (
      ...
      stats &&
      stats.buffered !== undefined
    ) {
      inner += `<p><b>Buffered:</b> ${this.getTotalBuffer(
        stats.buffered,
      )} s</p>`;
    }

    if (statsConfig.showStartupTime !== false && stats && stats.loadTime > 0) {
      inner += `<p><b>Startup time:</b> ${
        Math.round(stats.loadTime * 100) / 100
      } s</p>`;
    }
    ...
    if (
      ...
      inner += `<p><b>Bitrate:</b> ${Math.round(
        rendition.bitrate / 1000,
      )} Kbps</p>`;
    }
    ...
    const statsContainer = this.container.getElementsByClassName(
      `${Comparator.LIB_PREFIX}${player}-stats`,
    )[0] as HTMLDivElement;
    statsContainer.innerHTML = inner;
  }

  private getTotalBuffer(buffered: { start: number; end: number }[]): number {
    let res = 0;
  ...
  private seekInner($event: { offsetX: number }): void {
    const seekBar = this.container.getElementsByClassName(
      `${Comparator.LIB_PREFIX}seek-bar`,
    )[0] as HTMLDivElement;
    const seekBarInner = this.container.getElementsByClassName(
      `${Comparator.LIB_PREFIX}seek-bar-inner`,
    )[0] as HTMLDivElement;
    const time =
      ($event.offsetX * this.leftPlayerData.duration) / seekBar.offsetWidth;
    seekBarInner.style.width =
      (time / this.leftPlayerData.duration) * 100 + '%';
    this.seek(time);
  }
  ...
  private createVideoComparator(): void {
    ...
    this.leftPlayer = newPlayer(
      this.config.leftUrl,
      leftVideoWrapper.getElementsByTagName('video')[0],
      this.leftPlayerData.config,
    );
    this.rightPlayer = newPlayer(
      this.config.rightUrl,
      rightVideoWrapper.getElementsByTagName('video')[0],
      this.rightPlayerData.config,
    );
  ...
  private createMediaControls(): HTMLDivElement {
    ...
    this.container
      .getElementsByClassName(`${Comparator.LIB_PREFIX}wrapper`)[0]
      .appendChild(qualitySelectorPopupWrapper);
    ...
    qualitySelectorIcon.onclick = ($event) =>
      this.onQualityIconClick(
        $event,
        qualitySelectorIcon,
  ...
  private setPidController() {
    const target =
      this.leftPlayer.playerType === this.rightPlayer.playerType
        ? 0
  ...
  private showSpinner(): void {
    this.container
      .getElementsByClassName(`${Comparator.LIB_PREFIX}loading-spinner`)[0]
  ...
  private populateQualitySelector(): void {
    ...
    const popup = this.container.getElementsByClassName(
      `${Comparator.LIB_PREFIX}quality-selector-popup`,
    )[0];
    ...
    this.populateQualitySelectorSide(
      this.leftPlayer,
      this.leftPlayerData,
  ...
  private populateQualitySelectorSide(
    ...
    const [renditions, currentRendition] = [
      player.getRenditions(),
      player.getCurrentRendition(),
    ];
    ...
    if (
      data.config.initialRenditionIndex === Comparator.DEFAULT_QUALITY_INDEX &&
      data.config.initialRenditionKbps === Comparator.DEFAULT_QUALITY_KBPS
    ) {
      data.config.initialRenditionIndex = renditions.length - 1;
      data.config.initialRenditionKbps =
        renditions[renditions.length - 1].bitrate / 1000;
    }

    const listItemAuto = document.createElement('li');
    listItemAuto.innerHTML = `${
      data.config.initialRenditionIndex === -1 ? '> ' : ''
    }Auto`;
    ...
    for (let i = 0; i < renditions.length; i++) {
      ...
      const [width, height, kbps] = [
        renditions[i].width,
        renditions[i].height,
        Math.round(renditions[i].bitrate / 1000),
      ];
      listItem.innerHTML = `${selected}${width}x${height} (${kbps} kbps)`;
      listItem.className =
        currentRendition && renditions[i].bitrate === currentRendition.bitrate
          ? 'current'
          : '';
      listItem.onclick = () =>
        this.setRendition(player, i, renditions[i].bitrate);
  ...
  private setRendition(
    ...
    player.config.initialRenditionKbps =
      bitrate >= 0 ? Math.round(bitrate / 1000) + 1 : -1;
  ...
  private initListeners(): void {
    ...
    this.leftPlayer.htmlPlayer.addEventListener(
      'canplaythrough',
    ...
    this.rightPlayer.htmlPlayer.addEventListener(
      'canplaythrough',
      this.onCanPlayThrough,
    );
    ...
    const wrapper = this.container.getElementsByClassName(
      `${Comparator.LIB_PREFIX}wrapper`,
    )[0] as HTMLDivElement;
    const popupWrapper = this.container.getElementsByClassName(
      `${Comparator.LIB_PREFIX}quality-selector-popup-wrapper`,
    )[0];
    const leftStatsWrappers = this.container.getElementsByClassName(
      `${Comparator.LIB_PREFIX}left-stats`,
    )[0];
    const rightStatsWrappers = this.container.getElementsByClassName(
      `${Comparator.LIB_PREFIX}right-stats`,
    )[0];

    const moveSplit = (event: MouseEvent) => {
      if (!this.isSplitterSticked) {
        const leftWrapper = wrapper.getElementsByClassName(
          `${Comparator.LIB_PREFIX}left-video-wrapper`,
        )[0] as HTMLDivElement;
        leftWrapper.style.width = event.offsetX + 'px';
        leftWrapper.getElementsByTagName('video')[0].style.width =
          wrapper.offsetWidth + 'px';
      }
    };

    const stickSplit = (event: MouseEvent) => {
    ...
    // wrapper.ontouchstart = moveSplit; // TODO
    // wrapper.ontouchmove = moveSplit; // TODO
    ...
    if (this.config.stats !== false) {
      ...
      this.updateStatsBox(
        'left',
        this.leftPlayer.getStats(),
        this.leftPlayer.getCurrentRendition(),
      );
      this.updateStatsBox(
        'right',
        this.rightPlayer.getStats(),
        this.rightPlayer.getCurrentRendition(),
      );
  ...
  private destroyListeners(): void {
    ...
    this.leftPlayer.htmlPlayer.removeEventListener(
      'canplaythrough',
      this.onCanPlayThrough,
    );
    ...
    this.rightPlayer.htmlPlayer.removeEventListener(
      'canplaythrough',
      this.onCanPlayThrough,
    );
  ...
  private onCanPlayThrough = (evt: Event): void => {
    if (
      !this.leftPlayerData.isInitialized ||
      !this.rightPlayerData.isInitialized
    ) {
      if (
        (evt.target as HTMLVideoElement).classList.contains(
          `${Comparator.LIB_PREFIX}left-video`,
        )
      ) {
  ...
  private onSeeked = (evt: Event): void => {
    const player = (evt.target as HTMLVideoElement).classList.contains(
      `${Comparator.LIB_PREFIX}left-video`,
    )
      ? 'left'
      : 'right';
    if (
      (player === 'left' && !this.rightPlayer.htmlPlayer.seeking) ||
      (player === 'right' && !this.leftPlayer.htmlPlayer.seeking)
    ) {
      this.play();
    } else {
      this.pause();
  ...
  private onPlay = (): void => {
    const playPause = this.container.getElementsByClassName(
      `${Comparator.LIB_PREFIX}play-pause`,
    )[0] as HTMLDivElement;
  ...
  private onTimeUpdate = (): void => {
    ...
    if (seekBarInner !== undefined) {
      seekBarInner.style.width =
        (leftCurrentTime / this.leftPlayerData.duration) * 100 + '%';
    }
  ...
  private updatePlayersData(): boolean {
    const lChanged = this.updatePlayerData(
      this.leftPlayer,
      this.leftPlayerData,
    );
    const rChanged = this.updatePlayerData(
      this.rightPlayer,
      this.rightPlayerData,
    );
  ...
  private updatePlayerData(
    ...
    if (currentRendition) {
      changed =
        data.currentBitrate !== currentRendition.bitrate ? true : changed;
    ...
    changed =
      this.areEqualRenditions(renditions, data.renditions) === false
        ? true
        : changed;
  ...
  private areEqualRenditions(
    ...
    if (rend1.length > 0 && rend2.length > 0) {
      for (let i = 0; i < rend1.length; i++) {
        if (
          rend1[i].bitrate !== rend2[i].bitrate &&
          rend1[i].level !== rend2[i].level &&
          rend1[i].height !== rend2[i].height
        ) {
          return false;
        }
      }
  ...
  private toggleFullScreenClasses(): void {
    this.container
      .getElementsByClassName(`${Comparator.LIB_PREFIX}wrapper`)[0]
      .classList.toggle('full-screen-mode');
    this.container
      .getElementsByClassName(`${Comparator.LIB_PREFIX}media-controls`)[0]
      .classList.toggle('full-screen-mode');
    ...
    if (this.isFullScreen === true) {
      ...
      (
        this.container.getElementsByClassName(
          `${Comparator.LIB_PREFIX}media-controls`,
        )[0] as HTMLDivElement
      ).style.maxWidth = maxWidth;
    } else {
      (
